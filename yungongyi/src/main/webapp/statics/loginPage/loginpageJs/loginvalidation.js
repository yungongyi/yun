$(function() {
	...
	$("#login").click(function(){
		...
		if(flag==true){
			...
			$.post("/commonweal/login/doLogin.html",{iphone:phone,password:password},function(data){
				data=JSON.parse(data);
				if(data.id!=null){
					window.location.href="/yungongyi/loginSuccess.html/"+data.id;
				}else{
					$(".pwdError").text("用户名或密码错误！");
				}
	...
	$("#flogin").click(function() {
		...
		var  fyanzhengma=$('input[name=fyanzhengma]').val();
	
		$.post("/commonweal/login/fastLogin.html",{fphone:fphone,fyanzhengma:fyanzhengma},function(data){
			data=JSON.parse(data);
			if(data.id==null){
				$(".pwdError").text("用户名或密码错误");
			}else{
